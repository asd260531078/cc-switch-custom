...
pub(crate) fn build_provider_from_request(
    ...
    // Optional presentation metadata must never block configuration import.
    // Direct IPC imports pass the same validation as parsed deep links.
    if let Some(icon_url) = request
        .icon_url
        .as_ref()
        .filter(|value| crate::provider_logo::validate_icon_url(value).is_ok())
    {
        meta.get_or_insert_with(ProviderMeta::default).icon_url = Some(icon_url.clone());
    }
    ...
    if matches!(app_type, AppType::ClaudeDesktop) {
        let routes = crate::commands::suggested_claude_desktop_deeplink_routes(&provider)
            .unwrap_or_default();
        let direct = routes
            .values()
            .all(|route| crate::claude_desktop_config::is_claude_safe_model_id(&route.model));
        let routes = if direct {
            // Reuse the official duplicate/label merge before re-keying by model.
        ...
        // Direct profiles expose actual Claude IDs; local routing exposes the
        // official safe role IDs and maps them to the requested upstream models.
...
fn extract_claude_config_env(
    ...
    if format == "toml" && request.app.as_deref() == Some("claude-desktop") {
        let decoded = decode_base64_param("config", config_b64).ok()?;
        let value: toml::Value = toml::from_str(std::str::from_utf8(&decoded).ok()?).ok()?;
        let value = serde_json::to_value(value).ok()?;
        return value.get("env").and_then(|v| v.as_object()).cloned();
    }
    if format != "json" {
...
pub fn parse_and_merge_config(
    ...
    if normalized.resource == "provider"
        && matches!(
            normalized.app.as_deref(),
    ...
    match request.app.as_deref().unwrap_or("") {
        ...
        "claude-desktop" => {
            ...
            // Translate the standard Claude API-key field to Desktop's Bearer token.
            if merged.api_key.as_ref().is_none_or(|key| key.is_empty()) {
