...
| `app` | 是 | 应用类型：`claude` / `codex` / `gemini` / `opencode` / `openclaw` / `pi` |
| `name` | 是 | 名称 |

Pi 供应商导入必须显式提供 `name`、`endpoint`、`apiKey` 和 `model`，使用 OpenAI Chat Completions 接口，启用不会更改默认模型。端点与启用语义见 [Pi 网站接入契约](../../../pi-deeplink-import-zh.md)。
...
| `homepage` | 否 | 供应商官网 |
| `model` | 否 | 默认模型 |
| `models` | 否 | Codex 供应商的模型列表，模型 ID 用逗号分隔；仅列出该 API 端点实际支持的模型。未指定 `model` 时使用列表首项作为默认模型 |
| `haikuModel` | 否 | Haiku 模型（仅 Claude） |
| `sonnetModel` | 否 | Sonnet 模型（仅 Claude） |
| `opusModel` | 否 | Opus 模型（仅 Claude） |
| `notes` | 否 | 备注 |
| `icon` | 否 | 内置图标名称（保留原有兼容行为） |
| `iconUrl` | 否 | 网站 Logo 的绝对 HTTPS 图片地址，最长 2048 字符，不含用户名/密码；使用 URLSearchParams 编码一次 |
...
网站 Logo 使用传入的 `iconUrl`，不会根据 API 地址推断。路径、查询参数大小写及 URL 内的百分号编码原样保留。不同分站即使共用 API 地址，也分别保存自己的 Logo。旧链接可以继续只使用 `icon`。

Logo 支持 PNG、JPEG、WebP、GIF 和 ICO（动画取静态图像）。下载只访问公网 HTTPS，逐跳验证重定向；不携带供应商 API Key、Cookie 或认证头。下载限制 2 MiB，最多 3 次重定向，整个加载最多 12 秒；图片解码限制 2048×2048，解码分配预算 32 MiB，再转为最长边 256 像素的 PNG。SVG、HTML、无效、超时或无法缓存的图片会回退到原图标，不影响导入、切换和模型请求。

缓存通过 Tauri `app_cache_dir()` 获取，Windows/macOS 使用各自应用缓存目录；以完整原始 URL 的 SHA-256 区分，保留最多 128 张、有效期 7 天。编辑供应商时可修改或清除“网站 Logo 地址”。清除地址后立即恢复原有图标；升级无需迁移数据库，旧版会忽略新增的 `meta.iconUrl` 字段。
...
Codex 供应商可以同时提供默认模型和模型列表；CC Switch 导入后会为该供应商生成 Codex 模型目录。旧链接只有 `model` 时仍按原方式导入，不会据此推测端点还支持哪些模型。

```text
ccswitch://v1/import?resource=provider&app=codex&name=My%20Relay&endpoint=https%3A%2F%2Fapi.example.com%2Fv1&apiKey=sk-xxx&model=gpt-6-sol&models=gpt-6-sol%2Cgpt-6-luna
```
